...
app.get("/api/users", verifyToken, async (req, res) => {
    try {
      // 1. Authorization: Only allow admins to fetch all users
      if (req.user.role !== "admin") {
        // If a non-admin somehow bypasses frontend checks and hits this, they get 403
        return res.status(403).json({ message: "Access denied: Not an admin." });
      }
  
      // 2. Fetch all users with the 'employee' role
      // Ensure that you actually have users with role: 'employee' in your database.
      // Also, ensure the 'role' field in your UserSchema in db.js is consistently 'employee' (lowercase).
      const employees = await User.find({ role: "employee" }).select("_id email role");
  
      // 3. Log the fetched employees to your backend console to verify
      console.log("Fetched employees for /api/users:", employees);
  
      // 4. Send the employees data as JSON
      res.status(200).json(employees);
    } catch (error) {
      console.error("Error fetching users for /api/users:", error);
